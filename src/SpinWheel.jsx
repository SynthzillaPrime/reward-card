...
function SpinWheel({ onReset }) {
  ...
  const spinWheel = () => {
    ...
    for (let i = 0; i < selectedIndex; i++) {
      angleSum += (PRIZES[i].weight / TOTAL_WEIGHT) * 360;
    }
    const prizeMiddle = angleSum + (prize.weight / TOTAL_WEIGHT) * 360 / 2;

    // 8-12 full rotations + land on prize (pointer at top = 0°)
    const spins = 8 + Math.random() * 4;
    const targetRotation = spins * 360 + (360 - prizeMiddle);
          ...
          >
            {slices.map((slice, i) => (
              <g key={i}>
                ...
                />
                {slice.showText && (
                  <g transform={`translate(${slice.textX}, ${slice.textY}) rotate(${slice.textRotation})`}>
                    <text
                      textAnchor="middle"
        ...
        <div style={styles.legend}>
          {PRIZES.filter(p => !p.showText).map((prize, i) => (
            <div key={i} style={styles.legendItem}>
              ...
              <span style={styles.legendText}>{prize.name}</span>
            </div>
          ))}
        </div>

        {selectedPrize && (
          <div style={styles.result}>{selectedPrize.name}</div>
        )}

        <button
          onClick={selectedPrize ? () => onReset(selectedPrize.name) : spinWheel}
          disabled={isSpinning}
          style={{
            ...styles.button,
